...
private:
    ...
    size_t block_size_ = 0;        // 每个完整 block 的大小 = header_size_ + data_size_
...
public:
    ...
    explicit MemoryPool(size_t block_count = 64, size_t user_size = 128)
          ...
          align_(alignof(std::max_align_t))
    {
        ...
        block_size_  = header_size_ + data_size_;
    ...
    void* allocate()
    {
        ...
        char* user_ptr = reinterpret_cast<char*>(header) + header_size_;
        return user_ptr;
    }
    ...
    void free(void* ptr)
    {
        ...
        if (!is_owned(ptr))
        {
            std::cout << "Error: pointer does not belong to this MemoryPool\n";
            return;
        }
